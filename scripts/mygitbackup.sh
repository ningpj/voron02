...
gitrepos=(~/klipper ~/moonraker ~/fluidd ~/Happy-Hare ~/KlipperScreen ~/klipper_z_calibration ~/klipper_tmc_autotune ~/klippain_shaketune ~/crowsnest)
...
for repo in "${gitrepos[@]}"; do
   ...
   if [ -d ${repo} ]; then                    
      if [ -e ${repo}/.version ]; then
         ...
         #rev="${trimedRepo} on commit: $(git -C ${repo} tag -l --sort=v:refname | tail -1)-$(git -C ${repo} rev-parse --short=7 HEAD)"
         rev="${trimedRepo} on commit: $(git -C ${repo} describe --always --tags --long | awk '{gsub(/^ +| +$/,"")} {print $0}')"
      fi
      gitrevs+=("-m \"${rev}\"")
      echo $rev
   fi
...
git pull -v "https://ningpj:$mygittoken@github.com/ningpj/voron02.git"
git add . -v
current_date=$(date +"%Y-%m-%d %T")
git commit -m "Backup triggered on $current_date" "${gitrevs[@]}"
git push "https://ningpj:$mygittoken@github.com/ningpj/voron02.git"
